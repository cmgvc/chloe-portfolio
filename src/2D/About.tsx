...
const IconCard = ({ children, hoverKey, className }: { children: ReactNode; hoverKey: string; className?: string }) => {
    const { bgStyles, ...handlers } = useCardHover(hoverKey);
    return (
        <div className={`duration-300 ${bgStyles} cursor-pointer shadow-lg rounded-lg p-6 ${className}`} {...handlers}>
            {children}
        </div>
    );
};
    ...
    return (
        <PageLayout label="About" title="About Me" divId="about-div">
            ...
            <div className="grid grid-cols-2 gap-4 mt-8 w-full max-w-4xl">
                {[
                    { icon: <MapPinIcon className="h-24 w-24" />, title: "Location", body: "Based in Ottawa & Toronto, Canada" },
                    { icon: <LanguageIcon className="h-24 w-24" />, title: "Languages", body: "English & French. Can show up in a Spanish-speaking country and not get lost. Aspiring Korean & Japanese speaker." },
                ].map((card, i) => (
                    <FadeInText key={card.title} delay={i * 0.25} className="col-span-2 lg:col-span-1">
                        <IconCard hoverKey={`${card.title}-${card.body}`} className="flex gap-4 w-full">
                            {card.icon}
                            <div>
            ...
            <FadeInText delay={0.5} className="my-4"><span className="font-bold text-2xl">Specializing In</span></FadeInText>
